"""
Coolmate.me additional page cloner.
Clones category, cart, and checkout pages using same technique as clone-v2.py.
...
def process_html(html, page_url, all_css_texts, captured_hrefs):
    ...
    # 11. Inject all captured CSS as a single large <style> block in <head>
...
def clone_category_page(pg):
    """Clone the category page (men's products listing)."""
    clone_page(pg, "https://www.coolmate.me/nam", "category.html")


def clone_cart_page(pg):
    """
    Clone the cart page by:
    ...
    """
    print(f"\n{'='*60}")
    print("PREPARING CART: Adding product to cart first")
    print(f"{'='*60}")
    ...
    # Click a size button (M or L)
    print("  Selecting size...")
    size_clicked = False
    try:
        # Try multiple selectors for size buttons
        size_selectors = [
            # Common patterns for size selection on e-commerce sites
            'button:has-text("M")',
            'button:has-text("L")',
        ...
        for selector in size_selectors:
            try:
                elements = pg.query_selector_all(selector)
                for el in elements:
                    text = el.inner_text().strip()
                    if text in ["M", "L", "XL"]:
                        el.click()
                        size_clicked = True
                        print(f"    Clicked size: {text}")
                        break
                if size_clicked:
                    break
            except Exception:
        ...
        if not size_clicked:
            ...
            size_clicked = pg.evaluate("""
                () => {
                    const allBtns = document.querySelectorAll('button, div[role="button"], span[role="button"], div[class*="size"], label');
                    for (const btn of allBtns) {
                        const text = (btn.innerText || btn.textContent || '').trim();
                        if (text === 'M' || text === 'L' || text === 'XL') {
                            btn.click();
                            return text;
                        }
                    }
                    // Try clicking any size-like element
                    const sizeEl = document.querySelector('[class*="size"]:not([class*="resize"])');
                    if (sizeEl) {
                        const items = sizeEl.querySelectorAll('div, span, button, label');
                        for (const item of items) {
                            const t = (item.innerText || '').trim();
                            if (['S', 'M', 'L', 'XL', 'XXL'].includes(t)) {
                                item.click();
                                return t;
                            }
                        }
            ...
            """)
            if size_clicked:
                print(f"    Clicked size via JS: {size_clicked}")
            else:
                print("    WARNING: Could not find size button, trying to add anyway")
    except Exception as e:
        print(f"    Size selection error: {e}")

    time.sleep(2)

    # Click "Them vao gio" (Add to cart) button
        ...
        for selector in atc_selectors:
            try:
                ...
                if btn and btn.is_visible():
                    ...
                    print(f"    Clicked via selector: {selector}")
                    break
            except Exception:
                continue
                ...
                () => {
                    ...
                    for (const btn of allBtns) {
                        ...
                        if (text.includes('thêm vào giỏ') || text.includes('them vao gio') ||
                            text.includes('add to cart') || text.includes('mua ngay') ||
                            text.includes('chọn mua')) {
                            btn.click();
                            return text;
                        }
                    }
    ...
    # Now navigate to cart page and clone it
    clone_page(pg, "https://www.coolmate.me/cart", "cart.html")


def clone_checkout_page(pg):
    """
    Clone the checkout page by navigating from cart to checkout.
    """
    print(f"\n{'='*60}")
    print("PREPARING CHECKOUT: Navigating from cart to checkout")
    print(f"{'='*60}")

    # Make sure we're on the cart page
    print("  Ensuring we're on cart page...")
    ...
    try:
        ...
        for selector in checkout_selectors:
            try:
                ...
                if btn and btn.is_visible():
                    ...
                    print(f"    Clicked checkout via: {selector}")
                    break
            except Exception:
                continue
        ...
        if not checkout_clicked:
            result = pg.evaluate("""
                () => {
                    ...
                    for (const el of allEls) {
                        ...
                        if (text.includes('thanh toán') || text.includes('checkout') ||
                            text.includes('đặt hàng') || text.includes('tiến hành') ||
            ...
            if result:
                checkout_clicked = True
                print(f"    Clicked checkout via JS: {result}")

        if not checkout_clicked:
            # Try direct navigation as fallback
            print("    Trying direct navigation to checkout...")
            pg.goto("https://www.coolmate.me/checkout", wait_until="domcontentloaded", timeout=120000)
            checkout_clicked = True
    except Exception as e:
        print(f"    Checkout navigation error: {e}")
    ...
    # Capture whatever page we're on (checkout, login prompt, etc.)
    current_url = pg.url
    print(f"  Current URL after checkout click: {current_url}")

    # Capture styles and HTML from current page
    all_css, captured_hrefs = capture_all_styles(pg, current_url)
    scroll_page(pg, max_scrolls=10, pause=0.3)
    force_lazy_images(pg)
    time.sleep(3)

    # Re-capture styles after scroll
    all_css, captured_hrefs = capture_all_styles(pg, current_url)
    ...
    result_html = process_html(html, current_url, all_css, captured_hrefs)
...
def main():
    print("=" * 60)
    print("COOLMATE.ME ADDITIONAL PAGE CLONER")
    print("  Category + Cart + Checkout pages")
    print("=" * 60)
    ...
    with sync_playwright() as p:
        ...
        # 1. Clone category page
        print("\n\n[1/3] CLONING CATEGORY PAGE")
        clone_category_page(pg)

        # 2. Clone cart page (requires adding product first)
        print("\n\n[2/3] CLONING CART PAGE")
        clone_cart_page(pg)

        # 3. Clone checkout page (navigate from cart)
        print("\n\n[3/3] CLONING CHECKOUT PAGE")
        clone_checkout_page(pg)
    ...
    print(f"  Category: {BASE_DIR / 'category.html'}")
    print(f"  Cart:     {BASE_DIR / 'cart.html'}")
    print(f"  Checkout: {BASE_DIR / 'checkout.html'}")
