"""
Coolmate.me checkout page cloner v2.
Coolmate uses a combined cart+checkout page at /cart.
This script adds a product, navigates to /cart, fills some checkout fields,
then captures the page focused on the checkout form section.
"""
...
def main():
    print("=" * 60)
    print("COOLMATE.ME CHECKOUT PAGE CLONER v2")
    print("  Coolmate uses combined cart+checkout at /cart")
    print("  This captures it with filled form fields")
    print("=" * 60)
    ...
    with sync_playwright() as p:
        ...
        pg.evaluate("""
            () => {
                ...
                for (const btn of allBtns) {
                    ...
                    if (text.includes('thêm vào giỏ') || text.includes('chọn mua') || text.includes('mua ngay')) {
                        btn.click();
                        return text;
                    }
                }
        ...
        # Step 3: Fill in checkout form fields to make it look like a real checkout
        print("\n[STEP 3] Filling checkout form fields...")
        try:
            # Fill name field
            name_input = pg.locator('input[placeholder*="họ tên"], input[placeholder*="Nhập họ tên"]')
            if name_input.count() > 0:
                name_input.first.fill("Nguyen Van A")
                print("    Filled name field")

            # Fill phone field
            phone_input = pg.locator('input[placeholder*="điện thoại"], input[placeholder*="số điện thoại"]')
            if phone_input.count() > 0:
                phone_input.first.fill("0901234567")
                print("    Filled phone field")

            # Fill email field
            email_input = pg.locator('input[placeholder*="email"], input[type="email"]')
            if email_input.count() > 0:
                email_input.first.fill("nguyenvana@email.com")
                print("    Filled email field")

            # Fill address field
            addr_input = pg.locator('input[placeholder*="địa chỉ"], input[placeholder*="Nhập địa chỉ"]')
            if addr_input.count() > 0:
                addr_input.first.fill("123 Nguyen Hue, Quan 1")
                print("    Filled address field")

            time.sleep(2)

            # Try to select city/province
            city_select = pg.locator('button:has-text("Chọn tỉnh"), select, [class*="province"], [class*="city"]')
            if city_select.count() > 0:
                city_select.first.click()
                time.sleep(1)
                # Try to select Ho Chi Minh City from dropdown
                hcm_option = pg.locator('text="Hồ Chí Minh"').first
                if hcm_option.is_visible():
                    hcm_option.click()
                    print("    Selected city: Ho Chi Minh")
                    time.sleep(1)

        except Exception as e:
            print(f"    Form fill error (non-critical): {e}")

        time.sleep(3)
        close_popups(pg)

        # Step 4: Capture the checkout page
        ...
        pg.screenshot(path=str(BASE_DIR / "_checkout_v2_debug.png"), full_page=True)
        print("  Saved debug screenshot: _checkout_v2_debug.png")
