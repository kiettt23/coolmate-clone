"""
Coolmate.me page cloner v2.
Captures the fully-rendered page with all dynamically-injected styles,
inlines critical CSS, downloads all assets, and produces offline HTML.

Key insight: Coolmate uses Next.js which injects CSS via JavaScript at runtime.
We must capture the <style> tags BEFORE removing scripts.
"""
...
def download_file(url, force_type=None):
    """Download a file and return local relative path."""
...
def download_and_rewrite_css(css_url):
    """Download external CSS file, rewrite its internal URLs."""
    if not css_url or css_url.startswith("data:"):
        return None
    if css_url.startswith("//"):
        css_url = "https:" + css_url
    if css_url in downloaded_map:
        return downloaded_map[css_url]
    ...
    try:
        ...
        rewritten = rewrite_css_urls(resp.text, css_base)

        fname = safe_filename(css_url, "css")
        fpath = CSS_DIR / fname
        fpath.write_text(rewritten, encoding="utf-8")

        rel = f"assets/css/{fpath.name}"
        downloaded_map[css_url] = rel
        return rel
    except Exception as e:
        print(f"    [CSS-ERR] {css_url[:70]}: {e}")
        return None
...
def capture_all_styles(page, page_url):
    ...
    This runs BEFORE we modify the DOM, so we get everything JS injected.
    Returns a list of CSS text strings with urls already resolved.
    """
    print("  Capturing all active stylesheets...")

    # Method 1: Get all stylesheets from document.styleSheets API
        ...
        () => {
            ...
            for (const sheet of document.styleSheets) {
                try {
                    ...
                    // Cross-origin stylesheets can't be read via JS
                    results.push({
                        href: sheet.href || null,
                        cssText: null,
                        ownerNodeTag: sheet.ownerNode ? sheet.ownerNode.tagName : null,
    ...
    for sd in style_data:
        if sd.get("cssText"):
            # Rewrite URLs relative to the page or the sheet's href
            ...
            # Cross-origin sheet: download it
            css_url = sd["href"]
            if css_url.startswith("//"):
                css_url = "https:" + css_url
            try:
...
def process_html(html, page_url, all_css_texts, captured_hrefs):
    ...
    - Remove scripts
    - Remove existing <style>/<link stylesheet> (we'll inject captured CSS)
    ...
    #    (we replace them with our captured CSS)
    for link in soup.find_all("link", rel=lambda r: r and "stylesheet" in r):
        link.decompose()
    for style in soup.find_all("style"):
        style.decompose()

    # 3. Remove Next.js data tags that are useless offline
    ...
    # 10. Inject all captured CSS as a single large <style> block in <head>
    ...
    # Combine all CSS into one big style tag
    combined_css = "\n\n".join(all_css_texts)
...
def clone_page(page, url, output_name):
    ...
    # CRITICAL: Capture all styles BEFORE we touch the DOM
...
def find_discount_product(page):
    """Find a product with discount/sale price on the listing page."""
    print("\n[FIND] Searching for discounted product on /nam...")
    page.goto(
        "https://www.coolmate.me/nam",
        wait_until="domcontentloaded",
        timeout=120000
    )
    time.sleep(8)
    scroll_page(page, max_scrolls=20, pause=0.3)

    url = page.evaluate("""
        () => {
            // Look for product cards with strikethrough/discount
            const allAnchors = document.querySelectorAll('a[href]');
            for (const a of allAnchors) {
                const href = a.href;
                if (!href.includes('/product/')) continue;
                const container = a.closest('[class*="product"]')
                    || a.closest('[class*="card"]')
                    || a.parentElement?.parentElement;
                if (!container) continue;

                const text = container.innerText || '';
                // Look for strikethrough price indicator
                const hasStrike = container.querySelector('del, s, [style*="line-through"], .line-through');
                const hasPercent = /-\\d+%/.test(text);

                if (hasStrike || hasPercent) {
                    return href;
                }
            }
            // Fallback: any product link
            const any = document.querySelector('a[href*="/product/"]');
    ...
    if url:
        ...
        print("  No discount product found, using fallback.")
        url = "https://www.coolmate.me/product/ao-khoac-the-thao-nam-flexline-active"
...
def main():
    print("=" * 60)
    print("COOLMATE.ME PAGE CLONER v2")
    print("  Captures runtime CSS for perfect offline rendering")
    print("=" * 60)
    ...
    with sync_playwright() as p:
        ...
        # Clone homepage
        clone_page(pg, "https://www.coolmate.me/", "index.html")

        # Find and clone discounted product
        product_url = find_discount_product(pg)
        clone_page(pg, product_url, "product.html")
    ...
    print(f"  Homepage: {BASE_DIR / 'index.html'}")
    print(f"  Product:  {BASE_DIR / 'product.html'}")
